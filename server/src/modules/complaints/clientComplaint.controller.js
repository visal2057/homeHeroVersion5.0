...
// the authorization check (the booking must actually belong to this client).
async function findBookingWithProvider(bookingId, clientUserId) {
  const { rows } = await query(
    `SELECT b.booking_id, b.provider_user_id, u.user_token AS provider_token
     FROM bookings b
     JOIN users u ON u.user_id = b.provider_user_id
     WHERE b.booking_id = $1 AND b.client_user_id = $2`,
...
export const submitClientComplaintHandler = asyncHandler(async (req, res) => {
  ...
  const booking = await findBookingWithProvider(Number(bookingId), complainantUserId);
  if (!booking) {
    throw new AppError('No booking with that ID was found for your account', 404);
  }

  // The provider token is optional now that the booking ID alone identifies
  // the provider - if given anyway, it must actually match that provider,
  // otherwise a mismatched token is a sign of a data-entry mistake.
  if (token && token.trim() && booking.provider_token.toUpperCase() !== token.trim().toUpperCase()) {
    throw new AppError('That provider token does not match the provider on this booking', 422);
  }
  ...
  const { rows } = await query(
    ...
    [complainantUserId, booking.provider_user_id, complaintDetails, booking.booking_id],
  );
